...
describe("API test", () => {
  ...
  it("should return 400 when paramter is NaN",async () => {
    const { status } = await api.get("/fibonacci?elements=NaN");
    expect(status).toBe(400);
  });

  it("should return 400 when paramter is not present",async () => {
    const { status } = await api.get("/fibonacci");
    expect(status).toBe(400);
  });

  it("should return 400 when paramter is not valid number",async () => {
